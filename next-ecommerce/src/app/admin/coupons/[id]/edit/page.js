...
export default function EditCouponPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/coupons/${params.id}`, { credentials: "include" })
      .then((r) => r.json())
      .then((data) => {
        ...
        setForm({
          ...
          discountUnit: d.discountUnit || "amount",
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setErrors({ submit: data.error || "Failed to update" });
        return;
      }
  ...
  if (!form) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/admin/coupons" className="mt-2 inline-block text-red-600 hover:underline">
          Back to Coupons
        </Link>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-3">
          <Link href="/admin/coupons" className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
            Cancel
          </Link>
          <button
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {COUPON_TYPES.map((t) => {
                ...
                return (
                  ...
                  >
                    {selected && <Check className="absolute top-2 right-2 w-5 h-5 text-red-600" />}

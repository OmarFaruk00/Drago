 ...
 * Shows "How can I help you?" / "আমি আপনাকে কিভাবে সাহায্য করতে পারি?" tooltip after a few seconds.
 */
...
export default function LiveChatWidget() {
  ...
  const tooltipText =
    locale === "bn" ? "আমি আপনাকে কিভাবে সাহায্য করতে পারি?" : "How can I help you?";
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setShowTooltip(true), TOOLTIP_DELAY_MS);
  ...
  return (
    <>
      <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end gap-2">
        {showTooltip && !isOpen && (
          <div
            className="bg-brand text-white text-sm px-4 py-2.5 rounded-lg shadow-lg max-w-[220px] text-center relative mr-2"
            role="tooltip"
          >
            {tooltipText}
            <span className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-full border-8 border-transparent border-l-brand" />
          </div>
        )}
        <button
        type="button"
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col items-center border-b-2 border-r-2 border-l-2 border-brand/30 rounded-b-xl">
        ...
        <p className="text-brand font-bold text-lg mb-1">{isBn ? "লগইন প্রয়োজন" : "Login Required"}</p>
        <p className="text-gray-700 text-sm text-center mb-4">
          {isBn ? "আমাদের সাপোর্ট টিমের সাথে লাইভ চ্যাট করতে লগইন করুন।" : "Log in to start live chat with our support team."}
        </p>
        <Link
          ...
          onClick={onClose}
        >
          {isBn ? "অ্যাকাউন্টে লগইন" : "Login to Account"}
        </Link>

        <div className="w-full flex items-center gap-2 my-4">
          <div className="flex-1 h-px bg-brand/50" />
          <span className="text-sm text-gray-600 font-medium">{isBn ? "অথবা" : "or"}</span>
          <div className="flex-1 h-px bg-brand/50" />
        </div>
        ...
        >
          ...
          {isBn ? "WhatsApp এ চ্যাট করুন" : "Chat With WhatsApp"}
        </a>
      </div>
    </div>
  );
...
function ChatWindowLoggedIn({ onClose }) {
  const { locale } = useLanguage();
  const isBn = locale === "bn";
  const greeting = isBn ? "হ্যালো! আজ আমরা আপনাকে কিভাবে সাহায্য করতে পারি?" : "Hi! How can we help you today?";
  const placeholder = isBn ? "মেসেজ লিখুন..." : "Type your message...";
  return (
    <div
      ...
      <div className="flex-1 overflow-y-auto p-4 min-h-[280px] bg-gray-50">
        <div className="flex items-start gap-2">
          ...
          <div className="bg-white px-3 py-2 rounded-lg rounded-tl-none shadow-sm border border-gray-100 max-w-[85%]">
            ...
            <p className="text-xs text-gray-400 mt-1">{isBn ? "এখনই" : "Just now"}</p>
          </div>
        </div>
        <p className="text-xs text-gray-500 mt-4 text-center">
          {isBn ? "চ্যাট ফিচার শীঘ্রই আসছে।" : "Chat functionality coming soon."}
        </p>
      </div>
